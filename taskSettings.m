...
Parameters.pathTask = fullfile(Parameters.baseLocation,'Seafile','replayTask'); % path to the task folder
...
Parameters.colorBlack = [0 0 0]; % rgb code for color black
...
Parameters.posLeft = Parameters.centerX - Parameters.centerX / 4; % position left of the response options
Parameters.posRight = Parameters.centerX + Parameters.centerX / 4; % position right of the response options
...
pattern = ['highspeedMRI_',Parameters.studyMode,'_sub',Parameters.subjectInfo.id]; % check study mode and id
...
if any(idx)
    ...
    maxSession = cellfun(@str2double,regexp(dataMatch,'(?<=sess)-*\d+','match')); % find the run numbers of the previous session
